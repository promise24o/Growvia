...
export function Sidebar({ mobileMenuOpen, toggleMobileMenu }: SidebarProps) {
  ...
  const isMenuOpen =
    mobileMenuOpen !== undefined ? mobileMenuOpen : internalMobileMenuOpen;
  ...
  const getPlanDetails = () => {
    ...
    const plan = organization[0].plan as SubscriptionPlan;
          ...
          <div className="flex items-center justify-between">
            <RouterLink href="/dashboard">
              <div className="flex items-center cursor-pointer">
                ...
                <h1 className="ml-3 text-xl font-semibold text-slate-800 dark:text-white font-heading">
                  Growvia
                </h1>
              </div>
            </RouterLink>
            ...
            {planDetails && (
              <div className="bg-primary-50 dark:bg-[#2a3042] rounded-lg p-4">
                ...
                <div className="mt-2 flex items-center text-xs text-slate-500 dark:text-slate-300">
                  <span>
                    {planDetails.limits.apps === 999999
                      ? "Unlimited"
                      : planDetails.limits.apps}{" "}
                    Apps
                  </span>
                  <span className="mx-2 text-slate-300 dark:text-slate-500">
                    |
                  </span>
                  <span>
                    {planDetails.limits.marketers === 999999
                      ? "Unlimited"
                      : planDetails.limits.marketers}{" "}
                    Marketers
                  </span>
                </div>
                <div className="mt-3">
