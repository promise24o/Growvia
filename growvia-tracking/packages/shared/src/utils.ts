...
export function generateDeviceFingerprint(context: Partial<EventContext>): string {
  const components = [
    context.userAgent || '',
    context.language || '',
...
export function extractAffiliateParams(url: string): {
  affiliateId?: string;
  campaignId?: string;
  clickId?: string;
} {
...
export function isWithinWindow(timestamp: number, windowSeconds: number): boolean {
  const now = Date.now();
...
export function isTrackingError(error: any): boolean {
  return error && error.name === 'TrackingError';
}

export function formatError(error: any): { message: string; code?: string; details?: any } {
