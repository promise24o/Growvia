...
export class MongoStorage implements IStorage {
  ...
  // Marketer Applications Methods
  async deleteMarketerApplication(id: string | number): Promise<boolean> {
    try {
      const result = await MarketerApplication.deleteOne({ _id: id });
      return result.deletedCount === 1;
    } catch (error) {
      console.error("Error deleting marketer application:", error);
      throw error;
    }
  }
  ...
  async updateMarketerApplication(
    id: string | number,
    data: Partial<any>
